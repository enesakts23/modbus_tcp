"""
C kodundan port edilmiş Modbus TCP Sunucusu
"""
import socket
import struct
...
class ModbusTCPServer:
    def __init__(self, host: str = "0.0.0.0", port: int = 1024):
        """C'deki modbus_new_tcp karşılığı"""
        self.host = host
        self.port = port
        self.socket = None
        self.mapping = None
...
def run_server():
    """C'deki main fonksiyonu karşılığı"""
    # Sunucu oluştur
    server = ModbusTCPServer()
    
    # Register ve coil'leri ayarla
    server.mapping = server.mapping_new(10, 10, 10, 10)
    
    # C kodundaki gibi başlangıç değerlerini ayarla
    # Coils
    server.mapping.tab_bits[2] = True
    server.mapping.tab_bits[3] = True
    server.mapping.tab_bits[8] = True
    server.mapping.tab_bits[9] = True
    
    # Discrete Inputs
    server.mapping.tab_input_bits[0] = True   # HIGH
    server.mapping.tab_input_bits[1] = False  # LOW
    server.mapping.tab_input_bits[2] = True   # HIGH
    server.mapping.tab_input_bits[6] = True
    server.mapping.tab_input_bits[7] = True
    server.mapping.tab_input_bits[9] = True
    
    # Holding Registers
    server.mapping.tab_registers[0] = 0x1234
    server.mapping.tab_registers[1] = 0x5678
    
    # Input Registers
    server.mapping.tab_input_registers[2] = 0x9ABC
    server.mapping.tab_input_registers[3] = 0xDEF1
    
    # Sunucuyu başlat
    if not server.tcp_listen():
        print("Socket açılamadı")
        return
        
    print(f"Modbus TCP sunucusu başlatılıyor (port {server.port})...")
    print("\nRegister Haritası:")
    
    print("\nCoils (0x01):")
    for i, value in enumerate(server.mapping.tab_bits):
        if value:
            print(f"  Adres {i}: ON")
            
    print("\nDiscrete Inputs (0x02):")
    for i, value in enumerate(server.mapping.tab_input_bits):
        if i in [0, 1, 2, 6, 7, 9]:
            print(f"  Adres {i}: {'HIGH' if value else 'LOW'}")
            
    print("\nHolding Registers (0x03):")
    for i, value in enumerate(server.mapping.tab_registers):
        if i in [0, 1]:
            print(f"  Adres {i}: 0x{value:04X}")
            
    print("\nInput Registers (0x04):")
    for i, value in enumerate(server.mapping.tab_input_registers):
        if i in [2, 3]:
            print(f"  Adres {i}: 0x{value:04X}")
    ...
    try:
        while True:
            ...
            client_socket = server.tcp_accept()
            ...
            while True:
                # İstemciden veri al
                query = server.receive(client_socket)
                if not query:
                    print("Bağlantı kapandı, yeni istemci bekleniyor...")
                    break
                    
                # Yanıt gönder
                server.reply(client_socket, query)
