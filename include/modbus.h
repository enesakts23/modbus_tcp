 ...
 * @param[in] value [write_single_req_value_t] COIL_ON or COIL_OFF value must
 * be used.
 * @return [modbus_req_return_val_t] returns request errors or MODBUS_REQ_OK
 * if no problem occured.
 */
...
/*****************************************************************************/
/**
 * @brief Sends write multiple registers or coils request to modbus server.
 * @details This function will be used with multiple register sequence. if it
 * is used as write multiple registers, every register is 16 bit(2 Bytes) so
 * data given must be (reg_count * 2) sized. if it is used as write multiple
 * coils, data given must be sized as big as to contain reg_count number of
 * bits. Ex: reg_count is 10 that means 2 byte will be used. First bit is
 * contained as first bit of first byte and 8th bit is the last bit of first
 * byte. 9th bits is first byte of second byte etc.
 * @param[in] modbus_func [modbus_functions_t] modbus function indicator, this
 * parameter will be writing to multiple registers or coils. 
 * FUNC_WRITE_MULTIPLE_COILS or FUNC_WRITE_MULTIPLE_REGISTERS. Otherwise it'll
 * return error MODBUS_REQ_INVALID_FUNCTION.
 * @param[in] start_address [uint16_t] first address of the registers those
 * wanted to be written.
 * @param[in] data [uint8_t *] value list that'll be written. if used as write 
 * multiple registers, all data are stored as 16 bit values. So every register
 * which is wanted to be manipulated must be set with 2 byte values. if used
 * as write multiple coils, for example: first bit is first byte's first bit
 * and 9th bit is second byte's first bit.
 ...
 * @param[out] value [write_single_req_value_t *] this will return COIL_ON
